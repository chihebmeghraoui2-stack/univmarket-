...
export default function SellerPortfolio() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div>
        <h1 className="text-2xl font-extrabold">{t("portfolio_title")}</h1>
        <p className="text-muted-foreground text-sm">{t("portfolio_desc")}</p>
      </div>
      {isLoading ? (
        <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-16 rounded-xl" />)}</div>
      ) : !portfolio?.length ? (
        <Card><CardContent className="py-16 text-center text-muted-foreground">{t("no_portfolio")}</CardContent></Card>
      ) : (
        <Card>
          <CardHeader><CardTitle>{t("my_publications")}</CardTitle></CardHeader>
          <CardContent className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  <th className="py-3 px-4 text-left text-xs text-muted-foreground uppercase">{t("service_title")}</th>
                  <th className="py-3 px-4 text-left text-xs text-muted-foreground uppercase">{t("notes")}</th>
